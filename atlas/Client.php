...
class DeviceAtlasCloudClient {
	...
	const LICENCE_KEY = 'please_enter_your_licence_key';
	...
	const SOURCE_COOKIE = 'cookie cache';
	const SOURCE_FILE_CACHE = 'file cache';
	const SOURCE_CLOUD = 'cloud';
	const SOURCE_NONE = 'none';
	const DA_HEADER_PREFIX = 'X-DA-';
	const API_VERSION = 'php/1.1';
	...
	private static $ESSENTIAL_HEADERS = array('HTTP_X_PROFILE',
												'HTTP_X_WAP_PROFILE',
	...
	private static $EXTRA_HEADERS = array('HTTP_CLIENT_IP',
	...
	private static function prepareRequestHeader($user_agent) {
		...
		// add on any client headers we want to forward to DA
		if (!empty($_SERVER)) {
			// do essential headers
			foreach (self::$ESSENTIAL_HEADERS as $header) {
				if (isset($_SERVER[$header])) {
					$headers_str .= self::convertHeader($header) . ": " . $_SERVER[$header] . "\r\n";
				}
			}

			// also add on any Opera headers as these may contain extra device information
			...
			// finally do extra "optional" headers
	...
	private static function setCaches($user_agent, $device_data, $source) {
		...
		if(self::USE_COOKIE_CACHE && $source!=self::SOURCE_COOKIE) {
			$expire = time() + self::CACHE_ITEM_EXPIRY_SEC;
			setcookie(self::COOKIE_NAME, $device_data, $expire);
		}
	...
	private static function getCachePath($md5) {
		...
		$base_path = self::getCacheBasePath();
	...
	/**
	 * Returns the path to save the file cache.
	 * It can be the default path or the assigned one through the CUSTOM_CACHE_DIR constant.
	 */
	public static function getCacheBasePath() {
		...
		return $base_path.DIRECTORY_SEPARATOR.self::CACHE_NAME.DIRECTORY_SEPARATOR;
	}
